...
namespace Sep.Git.Tfs.Test.Commands
{
    public class InitBranchTest
    {
        ...
        private void ShouldInitBranch(string expectedGitBranchName)
        {
            ...
            mockRemote.Stub(r => r.Fetch(-1, Arg<bool>.Is.Anything)).Return(new GitTfsRemote.FetchResult() {IsSuccess = true});
            ...
            newBranchRemote.Expect(r => r.Fetch(-1, Arg<bool>.Is.Anything)).Return(new GitTfsRemote.FetchResult() { IsSuccess = true }).Repeat.Once();
        ...
        public void ShouldInitAllBranches()
        {
            ...
            newBranch1Remote.Expect(r => r.Fetch(-1, Arg<bool>.Is.Anything)).Return(new GitTfsRemote.FetchResult() { IsSuccess = true }).Repeat.Once();
            ...
            newBranch2Remote.Expect(r => r.Fetch(-1, Arg<bool>.Is.Anything)).Return(new GitTfsRemote.FetchResult() { IsSuccess = true }).Repeat.Once();
            newBranch2Remote.MaxCommitHash = "ShaAfterFetch_Branch2";
            #endregion

            remote.Expect(r => r.Fetch(-1, Arg<bool>.Is.Anything)).Return(new GitTfsRemote.FetchResult() { IsSuccess = true }).Repeat.Once();
